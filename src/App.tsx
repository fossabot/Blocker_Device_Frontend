...
function App() {
  ...
  // 누락 알림 처리: 기존 Toast 처리 로직 재사용
  const handleMissedNotifications = (notifications: any[]) => {
    notifications.forEach((notification) => {
      if (!notification || !notification.type) return;
      // 기존 ws.lastNotification과 동일하게 처리
      ...
      if (type === 'new_update' && (data as Update).uid) {
        ...
        setUpdatesRefreshTrigger((v: number) => v + 1);
      }
      if (type === 'update_progress' && (data as UpdateProgress).uid) {
        const progressData = data as UpdateProgress;
        const { uid, progress = 0 } = progressData;
  ...
  return (
    <WebSocketProvider onMissedNotifications={handleMissedNotifications}>
      <div className="App">
        <NavBar />
        <AppRouter />
